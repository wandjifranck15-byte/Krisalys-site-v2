...
export default function BlogPage() {
  return (
    <section className="bg-krisalys-black py-20">
      <Container>
        <SectionHeading eyebrow="Blog" title="Ressources & actualités KRISALYS" light />
        <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {blogPosts.map((post) => (
            <Link
              key={post.slug}
              href={`/blog/${post.slug}`}
              className="group flex flex-col rounded-2xl border border-white/10 bg-krisalys-anthracite p-6 transition-all hover:-translate-y-1 hover:border-krisalys-blue/40"
            >
              ...
              <p className="mt-2 flex-1 text-sm text-krisalys-gray-light">{post.excerpt}</p>
